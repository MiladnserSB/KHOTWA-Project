import 'package:get/get.dart';

class ArabicLocalization extends Translations {
  @override
  Map<String, Map<String, String>> get keys => {
    'ar': {
      //AnimatedBottomBarPage
      "cart": "السلة",
      "login": "تسجيل الدخول",
      "continue as a guest": "متابعة كمضيف",
      "OR": "أو",
      "donate": "تبرع",
      "Donate": "تبرع",
      "menu": "القائمة",
      "my events": "فعالياتي",
      "home": "الرئيسية",
      "projects events": "المشاريع والفعاليات",
      "Events": "فعاليات",
      "Projects": "مشاريع",
      "tasks": "المهام",
      "Target money": "الهدف",
      "profile": "الملف الشخصي",
      "Donated": "تبرع",
      "Remaining": "متبقي",
      "settings": "الإعدادات",
      "about us": "حول",
      "logout": "تسجيل الخروج",
      "confirm logout": "تأكيد تسجيل الخروج",
      "logout message": "هل أنت متأكد أنك تريد تسجيل الخروج؟",
      "cancel": "إلغاء",
      "user name": "المستخدم",
      "user email": "User@gmail.com",
      //Settings
      "settings": "الإعدادات",
      "change mode": "تغيير الوضع",
      "dark mode": "الوضع الليلي",
      "light mode": "الوضع النهاري",
      "change language": "تغيير اللغة",
      "English": "الإنجليزية",
      "Arabic": "العربية",
      "Theme Changed": "تم تغيير النمط",
      "Language Changed": "تم تغيير اللغة",
      //HomePageVolunteer
      "Khotwa": "درجة",
      "education support": "دعم التعليم",
      "refugee assistance": "مساعدة اللاجئين",
      "community water well": "بئر مياه المجتمع",
      "organization charity": "جمعية خيرية",
      "organization relief": "مجموعة الإغاثة",
      "organization water": "مؤسسة المياه",
      "status accept": "مقبول",
      "my events": "فعالياتي",
      "Recommended Events": " فعاليات موصى بها",
      "top projects": "أهم المشاريع",
      "view all": "عرض الكل",
      "search": "بحث...",
      "View Details": "عرض تفاصيل",
      "Apology Confirmation": "تأكيد الاعتذار",
      "Are you sure you want to apologize for the event":
          "هل متأكد من الاعتذار ",
      "Event Details": "تفاصيل فعالية",
      "Details": "عرض تفاصيل",
      "paid": "المدفوع: @amount",
      "remaining": "المتبقي: @amount",
      //profile
      "Milad AlNser": "ميلاد النصير",
      "Edit": "تعديل",
      "Save": "حفظ",
      "Full Name": "الاسم الكامل",
      "Gender": "الجنس",
      "Male": "ذكر",
      "Female": "أنثى",
      "Email": "البريد الإلكتروني",
      "Phone": "رقم الهاتف",
      "Birth Date": "تاريخ الميلاد",
      "Address": "العنوان",
      "City": "المدينة",
      "Areas of Interest": "مجالات الاهتمام",
      "Availability": "التوافر",
      "Preferred Time": "الوقت المفضل",
      "Previous Experience": "الخبرات السابقة",
      "Skills": "المهارات",
      "Motivation": "الدافع",
      "Contact Name": "اسم جهة الاتصال",
      "Relationship": "العلاقة",
      "Home": "الرئيسية",
      "Menu": "القائمة",
      "Login": "تسجيل الدخول",
      "Donate": "التبرع",
      "Cart": "السلة",
      "Settings": "الإعدادات",
      "About Us": "من نحن",
      "Logout": "تسجيل الخروج",
      "John Doe": "جون دو",
      "Jane Doe": "جين دو",
      "Sister": "أخت",
      "Health, Education": "الصحة والتعليم",
      "Mon, Wed, Fri": "الإثنين، الأربعاء، الجمعة",
      "5-8 hours/week": "5-8 ساعات/أسبوع",
      "Blood donation volunteer": "متطوع تبرع بالدم",
      "First aid, Event organizing": "الإسعافات الأولية، تنظيم الفعاليات",
      "Helping my community": "مساعدة مجتمعي",
      "Volunteer History": "تاريخ المتطوع",
      "Emergency Contact": "جهة الاتصال للطوارئ",
      "Experience and Skills": "الخبرات والمهارات",
      "Volunteer Info": "معلومات المتطوع",
      "Personal Information": "المعلومات الشخصية",
      "Volunteer Hours": "ساعات التطوع",
      "Ahmed Ali": "أحمد علي",
      "Joined: March 15, 2022": "انضم: 15 مارس 2022",
      "Join": "انضم",
      "funded": "ممول",
      "Start Date": "وقت البداية",
      "End Date": "وقت النهاية",
      "Volunteer Histor": "تاريخ التطوع",
      "Volunteer Hourse": "ساعات التطوع",
      "Active Volunteer": "متطوع نشط",
      "Warnings": "التحذيرات",
      "Participated and Current Campaigns": "المشاركات والحملات الحالية",
      "Active": "نشط",
      "Completed": "اكمال",
      "Hours: ": " عدد الساعات: ",
      "Role: ": "الدور: ",
      //task
      "My Tasks": "مهامي",
      "Cancel": "إلغاء",
      "Confirm": "تأكيد",
      "In Progress": "قيد التنفيذ",
      "completed": "مكتمل",
      "Pending": "قيد التحميل",
      "Update Status": "تحديث الحالة",
      "Are you sure you want to update the task status?":
          "هل أنت متأكد أنك تريد تحديث حالة المهمة؟",
      "Decline Task": "رفض المهمة",
      "Are you sure you want to decline this task?":
          "هل أنت متأكد أنك تريد رفض هذه المهمة؟",
      "community outreach call": "الاتصال بالمراكز المجتمعية",
      "prepare registration":
          "تحضير مكاتب التسجيل وحزم الترحيب للمتطوعين الجدد.",
      "call community centers":
          "الاتصال بالمراكز المجتمعية لتأكيد المشاركة في حملة توزيع الطعام القادمة.",
      "Supervisor": "المشرف",
      "Event": "الفعالية",
      "Due": "تاريخ الاستحقاق",
      "Assigned": "مُسند",
      "Welcome event setup": "انشاء حدث",
      "create new task": "إنشاء مهمة جديدة",
      "title": "العنوان",
      "description": "الوصف",
      "enter task name": "أدخل اسم المهمة",
      "enter task description": "أدخل وصف المهمة",
      "start date": "تاريخ البداية",
      "due date": "تاريخ الاستحقاق",
      "assign": "إسناد",
      "no date selected": "لم يتم اختيار تاريخ",
      "Select Date": "اختر التاريخ",
      "Date": "تاريخ",
      "Time": "الوقت",
      "Location": "الموقع",
      "Volunteers": "المتطوعين",
      "Apologize": "اعتذار",
      "Project Details": "تفاصيل مشروع",
      "Status: Active": "حالة نشطة",
      "Upcoming Events": "الفعاليات القادمة",
      "Donation Progress": "ميزانية التبرع",
      "Collected:": "تم جمع",
      "Target:": "الهدف",
      "Donate Now": "تبرع الان",
      "Scan QR Code": "امسح رمز QR",
      "Scan a code": "امسح الرمز",
      "Scanned:": "مسح",
      "Result:": "النتيجة",
      "Login Required": "يتوجب تسجيل الدخول",
      "Please log in to continue with the donation process.":
          "سجل دخول لمتابعة عملية التبرع",
      "Login successful!": "تم تسجيل الدخول بنجاح",
      "Donor Login": "تسجيل دخول المتبرع",
      "Name": "الاسم",
      "Email": "البريد الالكتروني",
      "Enter your full name": "ادخل كامل الاسم",
      "Name must be at least 2 characters":
          " يجب الاسم على الاقل ان يكون محرفين",
      "Name is required": "الاسم ضروري",
      "Email is required": "البريد الالكتروني ضروري",
      "Enter a valid email": "ادخل بريد الكتروني صحيح",
      "Enter your email": "ادخل بريدك الالكتروني",
      "Error": "غلط",
      "Passwords do not match.": "كلمة السر غير متطابقة",
      "OTP is missing.": "رمز OTP غير موجود",
      "Please enter the new password and its confirmation.":
          "ادخل كلمة السر الجديدة",
      "New password is required": "كلمة السر الجديدة ضرورية ",
      "Password must be at least 6 characters":
          "يجب ان تكون كلمة السر على الاقل من 6 محارف",
      "Please confirm your password": "يجب تأكيد كلمة السر",
      "Passwords do not match": "كلمات السر غير متطابقة",
      "Changing Password": "تغيير كلمة السر",
      "New password": "كلمة السر الجديدة",
      "Password is required": "كلمة السر ضرورية",
      "Password": "كلمة السر",
      "Change": "تغيير",
      "Confirm New Password": "تأكيد كلمة السر الجديدة",
      "We are done": "خلصنا",
      "Please provide details for the warning": "يرجى تقديم تفاصيل التحذير",
      "Volunteer Feedback": "ملاحظات المتطوعين",
      "Warning": "تحذير",
      "Write warnings if any": "اكتب التحذيرات إن وجدت",
      "Warning Given": "تحذير معين",
      "Issue Warning": "تحذير من المشكلة",
      "Rate the Volunteer": "قيم المتطوع",

      "Punctuality": "الانضباط في المواعيد",
      "Work Quality": "جودة العمل",
      "Teamwork": "العمل الجماعي",
      "Initiative": "المبادرة",
      "Discipline": "الانضباط",
      "Initiated": "التوجيه",
      "Mentored": "الإرشاد",
      "Creative Contribution": "المساهمة الإبداعية",
      "Impactful": "مؤثر",
      "Inspirational": "ملهم",

      "Notes": "ملاحظات",
      "Write your notes here": "اكتب ملاحظاتك هنا",
      "Feedback submitted successfully!": "تم إرسال الملاحظات بنجاح!",
      "Yes": "نعم",
      "No": "لا",
      "Submit Feedback": "إرسال الملاحظات",
      "Volunteer Attendance": "حضور المتطوعين",
      "Check In": "دخول",
      "Check Out": "خروج",
      "QR Attendance": "تسجيل حضور QR",
      "Submit Attendance": "إرسال الحضور",
      "Check in QR": "تسجيل الدخول QR",
      "Scan this QR Checking in": "مسح QR لتسجيل الدخول",
      "Check out QR": "تسجيل الخروج QR",
      "Scan this QR Checking out": "مسح QR لتسجيل الخروج",
      "Feedback": "تقييم",
      "Donate": "تبرع",
      "Make a Difference Today": "اصنع فرقًا اليوم",
      "Your benevolent gift fuels positive change in communities worldwide.":
          "هديتك الكريمة تغذي التغيير الإيجابي في المجتمعات حول العالم.",
      "Every contribution, big or small, creates a ripple effect of hope and transformation.":
          "كل مساهمة، كبيرة كانت أم صغيرة، تخلق أثرًا متسلسلًا من الأمل والتحول.",
      "Enter your name": "ادخل اسمك",
      "Donor Name": "اسم المتبرع",
      "Donation Amount": "مقدار التبرع",
      "Enter valid amount": "ادخل مقدار صحيح",
      "Enter donation amount": "ادخل مقدار التبرع",
      "Cash": "كاش",
      "Card": "بطاقة",
      "Payment Method": "طريقة الدفع",
      "Select a payment method": "اختيار طريقة الدفع",
      "Donate Now": "تبرع الان",
      "My Donations": "تبرعاتي",
      "Donation Confirmation": "تأكيد التبرع",
      "Are you sure you want to donate for the event or project?":
          "هل أنت متأكد أنك تريد التبرع للحدث أو المشروع؟",
      "No donations found": "لم يتم العثور على التبرعات",
      "1. Example term 1": "١. مثال على البند ١",
      "2. Example term 2": "٢. مثال على البند ٢",
      "3. Example term 3": "٣. مثال على البند ٣",
      "4. Example term 4": "٤. مثال على البند ٤",
      "Sign in": "تسجيل الدخول",
      "enterYourEmailPlease": "ادخل البريد الالكتروني",
      "Sign in your account": " سجل دخول ",
      "We will send a recovery code to your email.":
          "سنرسل رمز تأكيد الى بريدك الالكتروني",
      "Logging in...": "جاري تسجيل الدخول",
      "forgotPassword?": "هل نسيت كلمة المرور؟",
      "checkOur": "تحقق لدينا",
      "termsPolicy": "وفق شروط السياسة",
      "close": "اغلاق",
      "termsAndConditions": "الشروط والأحكام",
      "send": "ارسال",
      "Campaign History": "تاريخ الحملة",
      "Supervisor:": "المشرف",
      "Event:": "الفعالية",
      "Notifications": "الاشعارات",
      "Notification Details": "تفاصيل الاشعارات",
      "Received:": "تاريخ الاستلام:",
      "EmailVerification": "تأكيد الايميل",
      "Enter the 5-Digit": "ادخل رمز من 5 أرقام",
      "Resend": "اعادة ارسال الرمز",
      "Don't Recieve Code": "لم يتم استلام الرمز",
      "WeAreAlmostThere": "نحن على وشك الوصول",
      "Verify": "تأكيد",
      "OK": "نعم",
      "Check Your Email Please!": "!تفحص بريدك",
      "A code was sent to @email.\nPlease open your inbox and enter it where needed.":
          "تم إرسال رمز إلى @email.\nيرجى فتح بريدك الوارد وإدخاله في المكان المطلوب.",
      "Details about you": "تفاصيل شخصية عنك",
      "Your volunteering details": "تفاصيل عن تطوعك",
      "Your skills and experience": "مهاراتك وتجاربك",
      "Emergency contact details": "معلومات اتصال الطوارئ",
      "History of your volunteering": "سجلك التطوعي",
      "Success": "بنجاح",
      "Profile saved": "تم حفظ معلوماتك",
      "My Profile": "ملفي الشخصي",
      "Event Location": "موقع الفعالية",
      "No results found": "لا يوجد نتائج بحث",
      "Search Results": "نتائج البحث",
      "about_us": "من نحن",
      "We are a volunteer organization dedicated to making a positive impact in our community.\n\nOur mission is to provide support and assistance to those in need, and to create opportunities for individuals to get involved and make a difference.":
          "نحن منظمة تطوعية مكرسة لإحداث تأثير إيجابي في مجتمعنا.\n\nمهمتنا هي تقديم الدعم والمساعدة لأولائك الذين هم في حاجة، وخلق فرص للأفراد للمشاركة والمساهمة في صنع فرق حقيقي.",

      "Target Donation": " المبلغ المطلوب",
      "Project Statistics": "احصائيات المشروع",
      "Remaining Amount": "المبلغ المتبقي",
      "Total Donations": "المبلغ الكامل",
      "Total Volunteers": "كامل عدد المتطوعين",
      "Total Events": "عدد الفعاليات",
      "Donated Amount": "مبلغ التبرع",
      "No results found": "لا يوجد نتائج",
      "Are you sure you want to do this action":
          "هل متأكد من اتمام هذه العملية ",
      "not started": "لم تبدأ بعد",
      "Supervisor ID": "رقم المشرف",
      "Withdraw": "انسحاب",
      "Task Completed": "تمت المهمة",
      "Accept": "قبول",
      "Reject": "رفض",
      "in_progress": "قيد الانجاز",
      "not_started": "لم تبدأ بعد",
      "Study": "الدراسة",
      "Career": "المهنة",
      "Interests": "الاهتمامات",
      "Volunteer Years": "عدد سنوات التطوع",
      "No changes": "لاتغيير",
      "Nothing was updated": "لم يتغير شيئ",
      "Choose from Gallery": "اختر من المعرض",
      "Take a Photo": "اخذ صورة فورية",
      "Failed to load profile": "خطأ في تحميل الصورة",
      "My Donation":"تبرعاتي ",
      "Write a comment":"اكتب تعليق",
      "Add Your Feedback":"اكتب ملاحظاتك",
      "Attendance":"الحضور",
      "My Events Calendar":"تقويم فعالياتي",
      "View Events Date in Calendar":"عرض فعالياتي في تقويم",
      "top Volunteers":"أبرز المتطوعين",
      "Volunteers":"المتطوعين",
      "top ":"أبرز",
      "register":"انشاء حساب"
    },
  };
}
